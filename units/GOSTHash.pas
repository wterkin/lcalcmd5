...
uses
  ...
  procedure Init;
  procedure Update( const Buf; Size: LongWord );
  procedure GetHash( var Digest );

{√Œ—“ –34.11-94.}
implementation

const
  MAX32: DWord = $FFFFFFFF;
  TABLES: array[0..3, 0..255] of DWord = (
    ( { table 1 }
...
procedure SHIFT61( var H: array of DWord; var V: array of DWord );
begin
...
procedure Round( K1: DWord; K2: DWord; var l: DWord; var r: DWord; var t: DWord );
begin
   t := K1 + r;
   l := l xor ( Tables[0,t and $FF] xor Tables[1,( t shr 8 ) and $FF] xor
                Tables[2,( t shr 16 ) and $FF] xor Tables[3,( t shr 24 ) and $FF] );
   t := K2 + l;
   r := r xor ( Tables[0,t and $FF] xor Tables[1,( t shr 8 ) and $FF] xor
                Tables[2,( t shr 16 ) and $FF] xor Tables[3,( t shr 24 ) and $FF] );
end;

procedure Gost( Data: array of DWord );
var
  U: array[0..7] of DWord;
  V: array[0..7] of DWord;
  W: array[0..7] of DWord;
  Key: array[0..7] of DWord;
  S: array[0..7] of DWord;
  i: Integer;
  Gost_L: DWord;
  Gost_R: DWord;
  Gost_T: DWord;
begin
   Move( State[0], {%H-}U[0], SizeOf( U ) );
   Move( Data[0], {%H-}V[0], SizeOf( V ) );
   i := 0;
   while i < 8 do
   begin
      // X(w, u, v)
      ...
      Gost_R := State[i];
      Gost_L := State[i+1];
      Round( Key[0], Key[1], Gost_L, Gost_R, Gost_T{%H-} );
      Round( Key[2], Key[3], Gost_L, Gost_R, Gost_T );
      Round( Key[4], Key[5], Gost_L, Gost_R, Gost_T );
      Round( Key[6], Key[7], Gost_L, Gost_R, Gost_T );
      Round( Key[0], Key[1], Gost_L, Gost_R, Gost_T );
      Round( Key[2], Key[3], Gost_L, Gost_R, Gost_T );
      Round( Key[4], Key[5], Gost_L, Gost_R, Gost_T );
      Round( Key[6], Key[7], Gost_L, Gost_R, Gost_T );
      Round( Key[0], Key[1], Gost_L, Gost_R, Gost_T );
      ...
      Round( Key[4], Key[5], Gost_L, Gost_R, Gost_T );
      Round( Key[6], Key[7], Gost_L, Gost_R, Gost_T );
      Round( Key[7], Key[6], Gost_L, Gost_R, Gost_T );
      Round( Key[5], Key[4], Gost_L, Gost_R, Gost_T );
      Round( Key[3], Key[2], Gost_L, Gost_R, Gost_T );
      Round( Key[1], Key[0], Gost_L, Gost_R, Gost_T );
      Gost_T := Gost_R;
      Gost_R := Gost_L;
      Gost_L := Gost_T;
      ...
      begin
         ...
         V[0] := V[4];
         V[2] := V[6];
         V[4] := Gost_L xor Gost_R;
         V[6] := V[0] xor Gost_R;
         Gost_L := V[1];
         Gost_R := V[3];
         V[1] := V[5];
         V[3] := V[7];
         V[5] := Gost_L xor Gost_R;
         V[7] := V[1] xor Gost_R;
      end;
...
procedure Transform( Inp: array of Byte );
var
  i, j: Integer;
...
begin
   ...
   while i < 8 do
   begin
      Data[i] := DWord( Inp[j] ) or ( DWord( Inp[j+1] ) shl 8 ) or
                 ( DWord( Inp[j+2] ) shl 16 ) or ( DWord( Inp[j+3] ) shl 24 );
      Save := State[i+8];
      State[i+8] := State[i+8] + Data[i] + Temp;
      if ( State[i+8] < Data[i] ) or ( State[i+8] < Save ) then
         Temp := 1
      else
         Temp := 0;
...
begin
   ...
   FillChar( Count[0], SizeOf( Count ), 0 );
   Len := 0;
...
var
  i, R: Integer;
  Tmp: array[0..32] of Byte;
  Tmp2: array[0..32] of Char;
  PBuf: ^Byte;
begin
   if ( MAX32-Count[0] ) < ( Size*8 ) then
   begin
      Count[1] := Count[1] + 1;
      Count[0] := MAX32 - Count[0];
      Count[0] := ( Size * 8 ) - Count[0];
   end
   else
      Count[0] := Count[0] + ( Size * 8 );
   if ( Len + Size ) < 32 then
   begin
      Move( Buf, Buffer[Len], Size );
      Len := Len + Size;
   end
   else
   begin
      ...
      begin
         i := 32 - Len;
         Move( Buf, Buffer[Len], i );
         Transform( Buffer );
      end;
      while ( DWord( i ) + 32 ) <= Size do
      begin
         PBuf := @Buf;
         Inc( PBuf, i );
         Move( PBuf^, {%H-}Tmp[0], 32 );
         Move( PBuf^, {%H-}Tmp2[0], 32 );
         Transform( Tmp );
         i := i + 32;
      end;
      PBuf := @Buf;
      Inc( PBuf, i );
      Move( PBuf^, Buffer[0], R );
      FillChar( Buffer[R], 32-R, 0 );
      Len := R;
   end;
end;
...
begin
   FillChar( {%H-}L[0], SizeOf( L ), 0 );

   if Len > 0 then
      Transform( Buffer );
   Move( Count[0], L[0], SizeOf( Count ) );
   Gost( L );
   Move( State[8], L[0], SizeOf( L ) );
   Gost( L );

   Move( State, Digest, 32 );
   Init;
end;
